...
s/redacted/redacted by SCFS/g;		#substitute "redacted" with "redacted by SEINet" so users dont think CCH deleted the information in locality and coordinate fields for some rare taxa
...
foreach ($informationWithheld){
	...
	if($informationWithheld=~m/field values redacted/i){ #if there is text in informationWithheld (case insensitive), use the string
	 &log_change("VOUCHER: specimen redacted by SCFS\t$id");
	 }
	 else{
	 $informationWithheld = "";
	 }
...
foreach ($catalogNumber){
	...
	s/scfs/SSCFS/;	#fix lower case errors
	s/^([0-9]+)/SCFS$1/;
}


$catnon = $catalogNumber;

#remove leading zeroes, 
	if($catalogNumber=~m/SCFS0+/){
		$catalogNumber=~s/SCFS0+/SCFS/;
	}
	else{
...
#check for nulls
if($catalogNumber !~ m/^SCFS\d+/){
	$catalogNumber = "SCFS$id"; #change to the SEINET unique record ID for the catalog number
	if($catalogNumber !~ m/^SCFS\d+/){
...
#Remove duplicates
if($seen{$catalogNumber}++){
	warn "Duplicate number: $catalogNumber<\n";
	&log_skip("Duplicate ID number, skipped:\t$catalogNumber");
	++$skipped{one};
	next Record;
}
...
if (($id =~ m/^(14294018|14294026)$/) && (length($TID{$tempName}) == 0)){ 
...
#fix additional problematic counties
	if(($id=~/^14626935$/) && ($county=~m/Selena/)){ #fix some really problematic county records
...
#use combined Lat/Long field format for SCFS
...
#Image: $IMG{$id} Image ID nulled because the links from NANSH are broken on their own website for SCFS.  Check to see if they are working when you do the next upload.
