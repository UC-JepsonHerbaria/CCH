...
#the new format within, processes the BAJA records file after the California records and appends the BAJA records and errors to the same output files.
############
my $mex_file="data_files/2018.01.05_RSA_CCH_Baja.txt";
my $file="data_files/2018.01.05_RSA_CCH.txt";
...
#Use barcode number for those with one
	if ($barcode =~ m/^(RSA|POM)\d/){ #check if barcode is NULL
				&log_change("ACC: ID field NULL, using barcode $id==>$barcode<\n");
		$id = $barcode;
...
#find duplicates
if($duplicate{$id}++){
	#$id =~ s/^(RSA|POM)(\d+[A-Z]?)$/$1$2D/g;
	print OUT2 "$id\tDUP\t$fullName\t$tempCounty\t$cchId\t$occurrenceID\n";
	#warn "Duplicate accession found: $id<\n";
	#&log_change("ACC: Duplicate accession number found, added a suffix, $id");
		++$skipped{one};
		next Record;
}
else{
	print OUT2 "$id\tNON\t$fullName\t$tempCounty\t$cchId\t$occurrenceID\n";
}

#change duplicates to alternative ID
	if ($id !~ m/^(RSA|POM)\d+/){ #check to see if modified ID matches the correct format
...
if (($id =~ m/^(RSA642293)$/) && ($county=~m/Angeles/) && ($longitude=~m/^115\./)){
